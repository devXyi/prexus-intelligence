    ...
    <div class="panel">
      <div class="panel-head">
        ...
        <div style="margin-left:auto;display:flex;gap:8px">
          ...
          <button onclick="window._met_port_toggle()" class="btn btn-ghost" style="padding:3px 12px;font-size:9px">+ Add Asset</button>
        </div>
      </div>
      <div id="port-add-form" style="display:none;padding:14px;border-bottom:1px solid var(--border);background:rgba(14,165,233,.03)">
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:12px">
          ${[['port-name','Asset Name *','text','Mumbai Storage Depot'],['port-type','Type','select',''],['port-cc','Country Code','text','IND'],['port-country','Country','text','India'],['port-lat','Latitude','number','18.93'],['port-lon','Longitude','number','72.83'],['port-val','Value (USD M)','number','10']].map(([id,lbl,t,ph])=>
            t==='select'?`<div><div class="form-label">${lbl}</div><select id="${id}" class="inp-field" style="font-size:12px;text-align:left"><option>Infrastructure</option><option>Energy</option><option>Manufacturing</option><option>Agriculture</option><option>Transport</option><option>Financial</option></select></div>`
            :`<div><div class="form-label">${lbl}</div><input id="${id}" type="${t}" placeholder="${ph}" class="inp-field" style="font-size:12px;text-align:left;letter-spacing:0"/></div>`).join('')}
        </div>
        <div style="display:flex;gap:8px">
          <button onclick="window._met_port_submit()" class="btn btn-primary" style="padding:7px 20px">Create Asset</button>
          <button onclick="window._met_port_toggle()" class="btn btn-ghost" style="padding:7px 16px">Cancel</button>
        </div>
      </div>
      <div style="overflow-x:auto">
        <table style="width:100%;border-collapse:collapse">
          ...
          <tbody>${sorted.map(_row).join('')}</tbody>
        </table>
      </div>
    </div>`;

  window._met_port_toggle=()=>{_showAdd=!_showAdd;const f=document.getElementById('port-add-form');if(f)f.style.display=_showAdd?'block':'none';};
  window._met_port_delete=async(id)=>{if(!confirm(`Delete ${id}?`))return;try{await deleteAsset(id);}catch{}; _assets=_assets.filter(a=>a.id!==id);store.set('assets',_assets);_render(container);};
  window._met_port_submit=async()=>{
    const name=document.getElementById('port-name')?.value?.trim();
    if(!name){return;}
    const type=document.getElementById('port-type')?.value;
    const cc=(document.getElementById('port-cc')?.value||'').toUpperCase();
    const country=document.getElementById('port-country')?.value||'';
    const lat=parseFloat(document.getElementById('port-lat')?.value||0);
    const lon=parseFloat(document.getElementById('port-lon')?.value||0);
    const value_mm=parseFloat(document.getElementById('port-val')?.value||10);
    try{
      const created=await createAsset({name,type,lat,lon,value_mm,cc,country});
      _assets.push(created);
    }catch{
      _assets.push({id:`${cc||'AST'}-${type?.slice(0,3).toUpperCase()||'AST'}-${Date.now().toString().slice(-4)}`,name,type,lat,lon,value_mm,cc,country,pr:0.5,tr:0.4,cr:0.45,alerts:0});
    }
    store.set('assets',_assets);_showAdd=false;_render(container);
  };
}
...
function _row(a){
  ...
  return `<tr style="border-bottom:1px solid rgba(14,165,233,.07);transition:background .12s"
    ...
    <td style="padding:8px 11px;font-family:var(--font-data);font-size:10px;color:var(--cobalt)">${a.id}</td>
    <td style="padding:8px 11px;font-size:11px;color:var(--text-primary);font-weight:500">${a.name}</td>
    <td style="padding:8px 11px;font-size:9px;color:var(--text-secondary);text-transform:uppercase">${a.type}</td>
    <td style="padding:8px 11px;font-size:10px;color:var(--text-secondary)">${a.cc||'—'}</td>
    ...
    <td style="padding:8px 11px">${(a.alerts||0)>0?`<span style="font-size:10px;color:var(--red);background:rgba(239,68,68,.1);border:1px solid rgba(239,68,68,.2);padding:1px 7px;border-radius:2px">${a.alerts}</span>`:'<span style="color:var(--text-muted);font-size:10px">—</span>'}</td>
    <td style="padding:8px 11px"><div style="display:flex;gap:5px">
      <span class="tag tag-cobalt" style="cursor:pointer">SCORE</span>
      <span class="tag tag-red" style="cursor:pointer" onclick="event.stopPropagation();window._met_port_delete('${a.id}')">DEL</span>
    </div></td>
  </tr>`;
}
